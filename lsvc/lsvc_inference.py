...
def format_lines(video_ids, predictions, top_k):
  ...
  for video_index in range(batch_size):
    ...
    yield video_ids[video_index].decode('utf-8') + "," + "".join("%i" % pair
                                                  for pair in line) + "\n"
  ...
  with tf.Session() as sess, gfile.Open(out_file_location, "w+") as out_file:
    ...
    try:
      while not coord.should_stop():
          ...
          out_file.write(video_id_batch_val[0] + ", ")

          for i in predictions_val[0]:
...
def main(unused_argv):
  ...
  if FLAGS.frame_features:
    reader = readers.LSVCFrameFeatureReader(feature_names=feature_names,
                                            feature_sizes=feature_sizes)
  else:
    reader = readers.LSVCAggregatedFeatureReader(feature_names=feature_names,
                                                 feature_sizes=feature_sizes)
