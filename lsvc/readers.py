...
class LSVCFrameFeatureReader(BaseReader):
  """Reads TFRecords of SequenceExamples.
  ...
  def get_video_matrix(self,
    ...
    #decode_ = tf.cast(tf.decode_raw(features, tf.uint8), tf.float32)
    decoded_ = tf.decode_raw(features, tf.float32)
    

    decoded_features = tf.reshape(decoded_, [-1, 1024])
    
    #decoded_features : 4096
    num_frames = tf.minimum(tf.shape(decoded_features)[0], max_frames)
    # feature_matrix = utils.Dequantize(decoded_features,
    #                                       max_quantized_value,
    #                                       min_quantized_value)
  ...
  def prepare_reader(self,
    ...
    """
    reader = tf.TFRecordReader()
    print ("filename_queue : {}".format(filename_queue))
    _, serialized_example = reader.read(filename_queue)
...
class LSVCAggregatedFeatureReader(BaseReader):
  """Reads TFRecords of pre-aggregated Examples.
